...
- **🤖 Automated Fixture & Score Sync:** A GitHub Actions workflow runs every 15 minutes and always syncs fixtures and scores from Football-Data.org — ensuring new rounds (e.g. knockout stage) are picked up as soon as they appear in the API. A diff check skips records that haven't changed, so already-locked matches are never overwritten.
...
| :--- | :--- | :--- |
| `.github/workflows/sync_matches.yml` | Every 15 minutes + manual | Always fetches the full fixture list and scores from Football-Data.org and upserts into `matches`. A diff check skips records with no changes; already-locked matches are never overwritten. |
...
│  • sync_matches (*/15): always syncs fixtures + scores      │
│    → Football-Data.org → Supabase (diff check, no rewrites) │
